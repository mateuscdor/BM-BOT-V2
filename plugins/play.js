...
let handler = async (m, { conn, command, text, usedPrefix }) => {
  if (!text) throw `*𖥨ํ∘̥⃟⸽⃟💎 Ingrese un texto o enlace de YouTube*\n\n*𖥨ํ∘̥⃟⸽⃟💎 Ejemplo:*\n*✦҈͜͡➳ ${usedPrefix + command} Bellyache*`
//  let chat = global.db.data.chats[m.chat]
  let results = await yts(text)
  let vid = results.all.find(video => video.seconds < 3600)
  if (!vid) throw '*𖥨ํ∘̥⃟⸽⃟💎 No pude encontrar el video, por favor intente ingresar el nombre original de la canción o video*'
